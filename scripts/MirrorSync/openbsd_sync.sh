...
target="/mnt/mirror/openbsd"

# Lockfile path
lock="/mnt/mirror/openbsd_sync.lck"
...
source_url='rsync://openbsd.c3sl.ufpr.br/openbsd/'

# An HTTP(S) URL pointing to the 'lastupdate' file on your chosen mirror.
lastupdate_url='https://openbsd.c3sl.ufpr.br/openbsd/timestamp'
...
if ! tty -s && [[ -f "$target/timestamp" ]] && diff -b <(curl -Ls "$lastupdate_url") "$target/timestamp" >/dev/null; then
	# keep lastsync file in sync for statistics generated by the Arch Linux website
	rsync_cmd "$source_url/timestamp" "$target/timestamp"
	exit 0
fi
...
#echo "Last sync was $(date -d @$(cat ${target}/timestamp))"
